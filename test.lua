-- [[ KRALLDEN SPY v9.5.3 - ULTIMATE SCROLL & RENDERING FIX ]] --
...
local MainMemory, PathFilter, ManualBannedPaths, AntiSpamCooldowns, AntiSpamCounts = {}, {}, {}, {}, {}
...
-- DETAILS SCROLL (НОВАЯ СИСТЕМА ПРОКРУТКИ)
...
DetailsScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; DetailsScroll.CanvasSize = UDim2.new(0, 0, 0, 0)
DetailsScroll.VerticalScrollBarInset = Enum.ScrollBarInset.ScrollBar -- Решает проблему перекрытия

local dLayout = Instance.new("UIListLayout", DetailsScroll) -- Заставляет скролл видеть высоту текста
...
local function getSafePath(obj)
    ...
    pcall(function()
        ...
        while t and t ~= game do
            ...
            local safeName = (n:match("^%d") or n:match("[%s%W]")) and '["'..n..'"]' or n
            if p == "" then p = safeName else if safeName:sub(1,1) == "[" then p = safeName .. "." .. p else p = safeName .. "." .. p end end
            t = t.Parent
        end
    end)
    return ("game." .. p):gsub("%.%[", "[") 
    ...
    if currentSelectionGUID then
        ...
        for path, data in pairs(ManualBannedPaths) do 
            if data.guid == currentSelectionGUID then 
                targetData = {path = path, guid = data.guid, isBanList = true}; foundInBanList = true; break 
            end 
        end
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        for i, d in ipairs(sortedMemory) do
            ...
            b.Size = UDim2.new(1, -6, 0, 30); b.LayoutOrder = i; b.Text = string.format("[%s]%s %s", d.type, (d.isSelf and " [S]" or ""), d.name); b:SetAttribute("GUID", d.guid); b:SetAttribute("IsSelf", d.isSelf)
...
local function createBotBtn(text, pos, size, color)
    ...
    b.Size = size or UDim2.new(0, 220, 0, 58); b.Position = pos; b.BackgroundColor3 = color; b.Text = text; b.TextColor3 = Color3.new(1,1,1); b.Font = Enum.Font.SourceSansBold; b.TextSize = 14; b.BorderSizePixel = 0
    return b
end
...
local function createTypeBtn(text, pos, state, color, varName)
    local b = Instance.new("TextButton", ContentFrame)
    b.Size = UDim2.new(0, 150, 0, 35); b.Position = pos; b.BackgroundColor3 = state and color or Color3.fromRGB(40, 40, 45); b.Text = text; b.TextColor3 = Color3.new(1,1,1); b.Font = Enum.Font.SourceSansBold; b.TextSize = 12; b.BorderSizePixel = 0
    b.MouseButton1Click:Connect(function()
        ...
        b.Text = varName.." SPY: "..(ns and "ON" or "OFF")
        b.BackgroundColor3 = ns and color or Color3.fromRGB(40, 40, 45)
    end)
end
