-- [[ KRALLDEN SPY v9.5.1 - FULL MEMORY & RENDERING RESTORATION ]] --

local player = game:GetService("Players").LocalPlayer
local playerGui = player:WaitForChild("PlayerGui")

-- Очистка старых версий (безопасно для всех экзекуторов)
...
-- Функция обратной связи (Feedback)
...
-- ИСПРАВЛЕННАЯ ФУНКЦИЯ ОТРИСОВКИ (Использует память и обходит лимиты)
local function updateDetailsView()
    ...
    -- Ищем в основной памяти
    for _, d in ipairs(MainMemory) do
        if d.guid == currentSelectionGUID then
            content = sortArgs and d.fullTextPretty or d.fullText
            found = true
            break
        end
    end
    ...
    if not found then
        for _, data in pairs(ManualBannedPaths) do
            if data.guid == currentSelectionGUID then
                content = sortArgs and (data.detailsPretty or data.details) or data.details
                found = true
                break
            end
        end
    ...
    if content ~= "" then
        -- Roblox TextBox ломается на ~16,384 символах. Режем до 15к для безопасности отрисовки.
        if #content > 15000 then
            content = content:sub(1, 15000) .. "\n\n--- [!] ТЕКСТ ОБРЕЗАН (ЛИМИТ ROBLOX). ИСПОЛЬЗУЙ 'COPY SCRIPT' ДЛЯ ПОЛНОГО КОДА ---"
        end
        
        Details.Text = content
        
        -- Принудительное обновление размера холста (фикс скролла)
        task.defer(function()
...
Title.Text = "KRALLDEN SPY v9.5.1"
...
local function addLog(rem, args, isSelf, typeLabel)
    ...
    local function parseValue(v, d, pretty, indent)
        ...
        elseif t == "table" then
            ...
            for k, val in pairs(v) do
                i = i + 1
                if i > 100000 then 
                    res = res .. (pretty and string.rep("  ", indent + 1) or "") .. "..." .. (pretty and "\n" or " ") 
                    break 
                end
        ...
        if m.path == eventPath and m.isSelf == isSelf then
            if isSelf then
                ...
                if controlMode then 
                    ...
                    if m.argsStr == finalArgsStr then alreadyExists = true; break end 
                end
            end
        end
    end
    ...
    -- Сдвиг таблицы для новых логов вверх
    for i = #MainMemory, 1, -1 do
        MainMemory[i + 1] = MainMemory[i]
    end
    MainMemory[1] = data
    
    if #MainMemory > 250 then
...
-- ВЗАИМОДЕЙСТВИЯ (Кнопки управления)
...
-- РЕНДЕР-ЦИКЛ (Обновление списка событий)
task.spawn(function()
