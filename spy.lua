-- [[ KRALLDEN SPY v9.5.3 - FINAL SCROLL & RENDERING REBORN ]] --
...
local selfMode, controlMode, antiSpam, spyFS, spyFC, spyIS = true, true, true, true, false, false
local sortEnabled, currentSelectionGUID, lastCount, isMin = false, nil, 0, false
...
-- Продвинутый визуализатор таблиц из 9.5.3
...
local function getSortedDetails(d)
    if not sortEnabled then return (d.prefix or "") .. d.fullText end
...
local function updateRedListUI()
    ...
    for path, data in pairs(ManualBannedPaths) do
        ...
        b.TextColor3 = Color3.new(1,1,1); b.Font = Enum.Font.SourceSansBold; b.TextSize = 10; b.BorderSizePixel = 0; b.Text = " [X] " .. (path:match("[^%.%[%]]+$") or path)
...
Title.Size = UDim2.new(0, 200, 1, 0); Title.BackgroundTransparency = 1; Title.Position = UDim2.new(0, 15, 0, 0); Title.Text = "KRALLDEN SPY v9.5.3"; Title.TextColor3 = Color3.new(1, 1, 1); Title.Font = Enum.Font.SourceSansBold; Title.TextSize = 16; Title.ZIndex = 11; Title.TextXAlignment = 0
...
-- НОВАЯ СИСТЕМА DETAILS С АВТО-ПРОРИСОВКОЙ
DetailsScroll = Instance.new("ScrollingFrame", ContentFrame)
DetailsScroll.Position = UDim2.new(0, 205, 0, 8); DetailsScroll.Size = UDim2.new(0, 448, 0, 255); DetailsScroll.BackgroundColor3 = Color3.fromRGB(10, 10, 12); DetailsScroll.BorderSizePixel = 0; DetailsScroll.ScrollBarThickness = 6; DetailsScroll.ClipsDescendants = true
DetailsScroll.AutomaticCanvasSize = Enum.AutomaticSize.Y; DetailsScroll.CanvasSize = UDim2.new(0, 0, 0, 0); DetailsScroll.VerticalScrollBarInset = Enum.ScrollBarInset.ScrollBar
...
local dPadding = Instance.new("UIPadding", Details); dPadding.PaddingLeft = UDim.new(0, 10); dPadding.PaddingRight = UDim.new(0, 10); dPadding.PaddingTop = UDim.new(0, 5); dPadding.PaddingBottom = UDim.new(0, 5)
...
local function getSafePath(obj)
    ...
    pcall(function()
        ...
        while t and t ~= game do
            ...
            local safeName = (n:match("^%d") or n:match("[%s%W]")) and '["'..n..'"]' or n
            p = (p == "") and safeName or (safeName:sub(1,1) == "[" and safeName .. "." .. p or safeName .. "." .. p)
            t = t.Parent
        end
    end)
    return ("game." .. p):gsub("%.%[", "[")
...
local function addLog(rem, args, isSelf, typeLabel)
    ...
    local function parseValue(v, d)
        ...
        elseif t == "table" then
            ...
            local res, i = "{", 0; for k, val in pairs(v) do i = i + 1; if i > 15 then res = res .. "... " break end; if isArray then res = res .. parseValue(val, d + 1) .. ", " else local key = type(k) == "number" and "["..k.."]" or '["'..tostring(k)..'"]'; res = res .. key .. " = " .. parseValue(val, d + 1) .. ", " end end
    ...
    if #MainMemory > 100 then table.remove(MainMemory, #MainMemory) end
end
...
ControlBtn.MouseButton1Click:Connect(function()
    ...
    AntiSpamBtn.Visible = not controlMode; BlockBtn.Visible = not controlMode; lastCount = -1
end)

DelBtn.MouseButton1Click:Connect(function()
    if currentSelectionGUID then
        local targetData, foundInBanList = nil, false
        for path, data in pairs(ManualBannedPaths) do if data.guid == currentSelectionGUID then targetData = {path = path, isBanList = true}; foundInBanList = true; break end end
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        for i, d in ipairs(sortedMemory) do
            local b = Instance.new("TextButton", Scroll); b.Size = UDim2.new(1, -6, 0, 30); b.LayoutOrder = i; b.Text = string.format("[%s]%s %s", d.type, (d.isSelf and " [S]" or ""), d.name); b:SetAttribute("GUID", d.guid); b:SetAttribute("IsSelf", d.isSelf)
...
SortBtn.MouseButton1Click:Connect(function()
    sortEnabled = not sortEnabled; SortBtn.Text = "SORT: "..(sortEnabled and "ON" or "OFF"); SortBtn.BackgroundColor3 = sortEnabled and Color3.fromRGB(0, 140, 140) or Color3.fromRGB(40, 70, 70)
    if currentSelectionGUID then
        for _, m in ipairs(MainMemory) do if m.guid == currentSelectionGUID then Details.Text = getSortedDetails(m) return end end
        for _, d in pairs(ManualBannedPaths) do if d.guid == currentSelectionGUID then Details.Text = getSortedDetails(d) return end end
    end
end)
...
ClearLogBtn.MouseButton1Click:Connect(function() local nM = {}; for _, m in ipairs(MainMemory) do if m.isSelf then table.insert(nM, m) end end; MainMemory = nM; lastCount = -1; feedback(ClearLogBtn, "CLEARED SRV") end)

local ClearSelfBtn = createBotBtn("CLEAR SELF", UDim2.new(0, 544, 0.68, 0), UDim2.new(0, 108, 0, 58), Color3.fromRGB(100, 80, 60))
ClearSelfBtn.MouseButton1Click:Connect(function() local nM = {}; for _, m in ipairs(MainMemory) do if not m.isSelf then table.insert(nM, m) end end; MainMemory = nM; lastCount = -1; feedback(ClearSelfBtn, "CLEARED SELF") end)
...
SelfBtn.MouseButton1Click:Connect(function() selfMode = not selfMode; lastCount = -1; SelfBtn.Text = "SELF: "..(selfMode and "ON" or "OFF"); SelfBtn.BackgroundColor3 = selfMode and Color3.fromRGB(45, 90, 45) or Color3.fromRGB(150, 50, 50) end)
