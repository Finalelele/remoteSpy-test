-- [[ KRALLDEN SPY v9.5.0 - BUFFER FIX & UI ADJUSTMENT ]] --
...
local function feedback(button, tempText)
    if not button or type(button) ~= "userdata" then return end -- Защита от кривых вызовов в оригинале
...
-- ВОЗВРАЩАЕМ КНОПКИ ХЕДЕРА НА МЕСТО (v9.4.8)
...
-- КНОПКА BUFFER ТЕПЕРЬ В DETAILS (ПРАВЫЙ ВЕРХНИЙ УГОЛ)
...
BufferBtn.MouseButton1Click:Connect(function()
    ...
    lastCount = -1 -- Рефреш лога при переключении
end)
