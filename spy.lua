...
local isMin, sortArgs = false, false
...
-- ФУНКЦИЯ ОБНОВЛЕНИЯ ПОЛЗУНКА (СКРОЛЛА)
local function forceUpdateCanvas()
    task.defer(function()
        if Details and DetailsScroll then
            -- Ждем отрисовки текста и берем его реальную высоту (Y)
            local realTextSizeY = Details.TextBounds.Y
            -- Устанавливаем размер холста под высоту текста + отступ
            DetailsScroll.CanvasSize = UDim2.new(0, 0, 0, realTextSizeY + 20)
        end
    end)
end
...
local function updateDetailsView()
    ...
    -- Вызываем перерасчет скролла сразу после смены текста
    forceUpdateCanvas()
end
...
local function addLog(rem, args, isSelf, typeLabel)
    ...
    local function parseValue(v, d, pretty, indent)
        ...
        elseif t == "table" then
            ...
            for k, val in pairs(v) do
                i = i + 1; if i > 100 then res = res .. (pretty and string.rep("  ", indent + 1) or "") .. "..." .. (pretty and "\n" or " ") break end
    ...
    local logDetailsPretty = string.format("Type: %s\nPath: %s\nArgs: %s\n\nScript:\n%s:%s(%s)", typeLabel, eventPath, (finalArgsStrPretty == "" and "None" or "\n" .. finalArgsStrPretty), eventPath, methodName, finalArgsStr)
        ...
        for i, d in ipairs(MainMemory) do
            if d.guid == currentSelectionGUID and not d.isSelf then
                ...
                if p then 
                    ...
                    Details.Text = "Banned."; feedback(BlockBtn, "BANNED") 
                end
                break
            end
        end
...
task.spawn(function()
    while task.wait(0.5) do
        ...
        for i, d in ipairs(sortedMemory) do
            ...
            b.Size = UDim2.new(1, -6, 0, 30); b.LayoutOrder = i
...
local function createBotBtn(text, pos, size, color)
    ...
    b.Position = pos; b.BackgroundColor3 = color
...
ExecuteBtn.MouseButton1Click:Connect(function() 
    ...
    if s and s ~= "" then 
        ...
        if f then 
            ...
            warn(err); feedback(ExecuteBtn, "ERROR!") 
        end 
    end 
end)
...
local function createTypeBtn(text, pos, state, color, varName)
    local b = Instance.new("TextButton", ContentFrame)
    b.Size = UDim2.new(0, 150, 0, 35); b.Position = pos
