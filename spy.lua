-- [[ KRALLDEN SPY v9.4.8 - MODDED WITH SORT & SCROLL ]] --
...
local sortEnabled = false -- Состояние сортировки
...
-- Функция форматирования (Красивые аргументы)
local function formatTable(val, indent)
    indent = indent or 0
    ...
    if t == "table" then
        ...
        for _, k in ipairs(keys) do
            ...
            res = res .. tab .. "    " .. keyStr .. " = " .. formatTable(v, indent + 1) .. ",\n"
        end
        return res .. tab .. "}"
...
-- КОРРЕКЦИЯ ДЕТАЛЕЙ: СКРОЛЛ + ТЕКСТБОКС
DetailsScroll = Instance.new("ScrollingFrame", ContentFrame)
DetailsScroll.Position = UDim2.new(0, 205, 0, 8); DetailsScroll.Size = UDim2.new(0, 448, 0, 255); DetailsScroll.BackgroundColor3 = Color3.fromRGB(10, 10, 12); DetailsScroll.BorderSizePixel = 0; DetailsScroll.AutomaticCanvasSize = Enum.AutomaticSize.XY

Details = Instance.new("TextBox", DetailsScroll)
Details.Size = UDim2.new(1, 0, 1, 0); Details.BackgroundTransparency = 1; Details.TextColor3 = Color3.new(1, 1, 1); Details.MultiLine = true; Details.TextWrapped = false; Details.TextEditable = true; Details.Font = Enum.Font.Code; Details.TextSize = 12; Details.TextXAlignment = 0; Details.TextYAlignment = 0; Details.ClearTextOnFocus = false; Details.AutomaticSize = Enum.AutomaticSize.XY
...
local function addLog(rem, args, isSelf, typeLabel)
    ...
    -- Сохраняем и оригинальные аргументы для форматирования
    local data = { 
        guid = generateGUID(), 
    ...
    while task.wait(0.5) do
        ...
        for i, d in ipairs(sortedMemory) do
            ...
            b.MouseButton1Click:Connect(function()
                ...
                if sortEnabled then
                    local prettyArgs = formatTable(d.rawArgs)
                    Details.Text = string.format("Type: %s\n\nPath: %s\n\nArgs: %s\n\nScript:\nlocal args = %s\n%s:%s(unpack(args))", d.type, d.path, prettyArgs, prettyArgs, d.path, d.method)
                else
...
-- РАЗДЕЛЕННЫЕ КНОПКИ COPY ARGS
local CopyArgsBtn = createBotBtn("COPY ARGS", UDim2.new(0, 205, 0.68, 0), UDim2.new(0, 108, 0, 58), Color3.fromRGB(45, 90, 45))
local SortToggleBtn = createBotBtn("SORT: OFF", UDim2.new(0, 317, 0.68, 0), UDim2.new(0, 108, 0, 58), Color3.fromRGB(80, 80, 85))

SortToggleBtn.MouseButton1Click:Connect(function()
    sortEnabled = not sortEnabled
    SortToggleBtn.Text = "SORT: " .. (sortEnabled and "ON" or "OFF")
    SortToggleBtn.BackgroundColor3 = sortEnabled and Color3.fromRGB(130, 70, 220) or Color3.fromRGB(80, 80, 85)
    
    -- Обновляем текущий текст в Details, если что-то выбрано
