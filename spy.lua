-- [[ KRALLDEN SPY v9.4.8 - FULL RESTORED + CLEAN SORT ]] --
...
-- Функция красивого форматирования (БЕЗ цифр в индексах)
local function formatTable(val, indent)
    ...
    elseif t == "Instance" then
        -- Попытка получить путь объекта
        local success, path = pcall(function() return val:GetFullName() end)
        return success and path or tostring(val)
    else
        return tostring(val)
    end
end
...
local function addLog(rem, args, isSelf, typeLabel)
    ...
    local data = { guid = generateGUID(), name = tostring(rem.Name), type = typeLabel, isSelf = isSelf, fullText = logDetails, path = eventPath, argsStr = finalArgsStr, rawArgs = args, method = methodName }
    ...
    while task.wait(0.5) do
        ...
        for i, d in ipairs(sortedMemory) do
            ...
            b.MouseButton1Click:Connect(function()
                ...
                if sortEnabled then
                    local pArgs = formatTable(d.rawArgs)
                    Details.Text = string.format("Type: %s\n\nPath: %s\n\nArgs: %s\n\nScript:\nlocal args = %s\n%s:%s(unpack(args))", d.type, d.path, pArgs, pArgs, d.path, d.method)
                else
...
local SortToggleBtn = createBotBtn("SORT: OFF", UDim2.new(0, 305, 0.68, 0), UDim2.new(0, 120, 0, 58), Color3.fromRGB(130, 70, 220))
...
SortToggleBtn.MouseButton1Click:Connect(function()
    ...
    SortToggleBtn.BackgroundColor3 = sortEnabled and Color3.fromRGB(100, 50, 200) or Color3.fromRGB(130, 70, 220)
    if currentSelectionGUID then
        for _, d in ipairs(MainMemory) do
            if d.guid == currentSelectionGUID then
                if sortEnabled then
                    local pArgs = formatTable(d.rawArgs)
                    Details.Text = string.format("Type: %s\n\nPath: %s\n\nArgs: %s\n\nScript:\nlocal args = %s\n%s:%s(unpack(args))", d.type, d.path, pArgs, pArgs, d.path, d.method)
                else Details.Text = d.fullText end
                break
            end
        end
    end
