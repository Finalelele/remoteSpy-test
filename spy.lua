-- [[ KRALLDEN SPY v9.5.0 - CLEAN VERSION WITH FULL SCROLL FIX ]] --

local player = game:GetService("Players").LocalPlayer
local playerGui = player:WaitForChild("PlayerGui")

-- Очистка старых версий (безопасно для Delta и других экзекуторов)
...
-- Функция фидбека (предотвращает зависание текста на кнопке)
...
-- Функция форматирования сортировки (Deep Table Format)
        ...
        pcall(function() 
            ...
            while t_obj and t_obj ~= game do 
                ...
                if p == "" then 
                    ...
                    p = (safeName:sub(1,1) == "[" and safeName .. "." .. p or safeName .. "." .. p) 
                end
                t_obj = t_obj.Parent 
            end 
        end)
...
-- [ИСПРАВЛЕННЫЙ SCROLLING FRAME ДЛЯ DETAILS]
...
DetailsScroll.CanvasSize = UDim2.new(0, 0, 0, 0) -- Важно для правильного расчета AutomaticSize
...
-- Добавляем Padding, чтобы текст не упирался в нижний край
local DetailsPadding = Instance.new("UIPadding", Details)
DetailsPadding.PaddingBottom = UDim.new(0, 20) -- Дает запас места снизу
    ...
    if currentSelectionGUID then
        ...
        if not found then
            for _, data in pairs(ManualBannedPaths) do
                if data.guid == currentSelectionGUID then 
                    Details.Text = getSortedDetails(data); break 
                end
            end
        end
    end
